...
namespace BWML {
	class RLMarineVsZealot : public Gym
	{
	private:
	...
	public:
		RLMarineVsZealot(string shmName, ConnMethod method = SHARED_MEMORY) : Gym("RLMarineVsZealot") {
			connection = new RLSharedMemory(shmName, 2000000);
			connection->initialize();
		}
		~RLMarineVsZealot() { };

		static RLMarineVsZealot &Instance(string shmName = "");
