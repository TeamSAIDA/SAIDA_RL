...
namespace BWEM {
	...
	namespace detail {
		...
		class MapImpl : public BWEMMap
		{
		public:
			// Downcast helpers
			static MapImpl 			*Get(BWEMMap *pMap)				{
				return static_cast<MapImpl *>(pMap);
			}
			static const MapImpl 		*Get(const BWEMMap *pMap)		{
				return static_cast<const MapImpl *>(pMap);
			}

			using BWEMMap::GetTile_;
			using BWEMMap::GetMiniTile_;
