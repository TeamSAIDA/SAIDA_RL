...
namespace BWEM {
	...
	namespace utils {
		...
		bool seaSide(WalkPosition p, const BWEMMap *pMap)
		{
	...
	unique_ptr<BWEMMap> BWEMMap::m_gInstance = nullptr;


	BWEMMap &BWEMMap::Instance()
	{
	...
	Position BWEMMap::RandomPosition() const
	{
	...
	TilePosition BWEMMap::Crop(const TilePosition &p) const
	{
		return crop(p, Size().x, Size().y);
	}


	WalkPosition BWEMMap::Crop(const WalkPosition &p) const
	{
		return crop(p, WalkSize().x, WalkSize().y);
	}


	Position BWEMMap::Crop(const Position &p) const
	{
