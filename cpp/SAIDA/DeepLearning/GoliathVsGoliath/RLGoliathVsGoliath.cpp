...
#include "RLGoliathVsGoliath.h"
#include "../../UXManager.h"

using namespace BWML;
using namespace MyBot;

RLGoliathVsGoliath &RLGoliathVsGoliath::Instance(string shmName) {
	static RLGoliathVsGoliath instance(shmName);
	return instance;
}

// Init
void RLGoliathVsGoliath::init(::google::protobuf::Message *message)
{
	...
	myType = Terran_Goliath;
	enType = Terran_Goliath;
}

bool RLGoliathVsGoliath::initializeAndValidate()
{
	if (myUnitSet.empty()) {
...
void RLGoliathVsGoliath::makeInitMessage(::google::protobuf::Message *initMessage)
{
	InitRes *message = (InitRes *)initMessage;

	message->set_num_action_space(ACTION_SIZE);

	setTypeInfo(message->add_unit_type_map(), Terran_Goliath);
...
// Reset
void RLGoliathVsGoliath::reset(bool isFirstResetCall)
{
...
bool RLGoliathVsGoliath::isResetFinished()
{
	return INFO.getUnits(myType, S).size() == MY_UNIT_COUNT && INFO.getUnits(enType, E).size() == ENEMY_UNIT_COUNT;
}

// Action
void RLGoliathVsGoliath::step(::google::protobuf::Message *stepReqMsg)
{
	if (myUnitSet.empty())
		return;
...
bool RLGoliathVsGoliath::isActionFinished()
{
	if (isDone())
		return true;
...
void RLGoliathVsGoliath::getObservation(::google::protobuf::Message *stateMsg)
{
...
// Done, Reward
bool RLGoliathVsGoliath::isDone()
{
	return isWin() || isDefeat();
}


float RLGoliathVsGoliath::getReward() {
...
bool RLGoliathVsGoliath::isDefeat() {
...
void RLGoliathVsGoliath::render()
{
...
void RLGoliathVsGoliath::onUnitDestroy(Unit unit) {
}
