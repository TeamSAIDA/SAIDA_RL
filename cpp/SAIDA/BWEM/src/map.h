...
namespace BWEM
{
	...
	public:
		...
		virtual								~BWEMMap() = default;

	protected:
		BWEMMap() = default;

		Tile 								&GetTile_(const BWAPI::TilePosition &p, utils::check_t checkMode = utils::check_t::check)		{
			return const_cast<Tile &>(static_cast<const BWEMMap &>(*this).GetTile(p, checkMode));
		}
		MiniTile 							&GetMiniTile_(const BWAPI::WalkPosition &p, utils::check_t checkMode = utils::check_t::check)	{
			return const_cast<MiniTile &>(static_cast<const BWEMMap &>(*this).GetMiniTile(p, checkMode));
		}
	...
	private:
		static std::unique_ptr<BWEMMap>	m_gInstance;

	};


	template<>
	inline const Tile &BWEMMap::GetTTile<BWAPI::TilePosition>(const BWAPI::TilePosition &t, utils::check_t checkMode) const
	{
		return GetTile(t, checkMode);
	}

	template<>
	inline const MiniTile &BWEMMap::GetTTile<BWAPI::WalkPosition>(const BWAPI::WalkPosition &w, utils::check_t checkMode) const
	{
	...
	template<class TPosition, class Pred1, class Pred2>
	inline TPosition BWEMMap::BreadthFirstSearch(TPosition start, Pred1 findCond, Pred2 visitCond, bool connect8) const
	{
	...
	namespace utils
	{
		...
		bool seaSide(BWAPI::WalkPosition p, const BWEMMap *pMap);
