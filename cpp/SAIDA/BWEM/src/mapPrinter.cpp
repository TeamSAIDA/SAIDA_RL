...
namespace BWEM {
	...
	namespace utils {
		...
		string MapPrinter::m_fileNamePath = "C:/StarCraft/bwapi-data/BWEM_map/";
		string MapPrinter::m_fileName = "";

		std::unique_ptr<BMP> MapPrinter::m_pBMP;
		MapPrinter MapPrinter::m_Instance;
		const BWEMMap *MapPrinter::m_pMap = nullptr;
		...
		void MapPrinter::Initialize(const BWEMMap *pMap)
		{
			bwem_assert_throw(pMap->Initialized());
			m_fileName = m_fileNamePath + BWAPI::Broodwar->mapFileName().c_str();
