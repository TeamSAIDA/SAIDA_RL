...
namespace BWEM {
	...
	namespace utils
	{
		...
		static bool canBuildWall(const BWEMMap &theMap, BWAPI::UnitType type, TilePosition location)
		{
		...
		static vector<TilePosition> findBuildableBorderTiles(const BWEMMap &theMap, WalkPosition cpEnd, const Area *area)
		{
		...
		{
		public:
			BorderTileInfo(const BWEMMap &theMap, const TilePosition &t)
				:
		...
		ExampleWall::ExampleWall(const BWEMMap &theMap, const ChokePoint *cp, tight_t tight)
		...
		vector<ExampleWall> findWalls(const BWEMMap &theMap)
		{
